...
public class Colonel{	
	...
	public int C_Col(Colonel e, LinkedList<Entity> ll){
		...
		Rectangle temp=e.getRec();
		for(int i=0; i < ll.size(); ++i)
		{
			if(temp.intersects(ll.get(i).getRec())){
	...
	public void setX(int x2){
		System.out.println("\t\t-> [:Colonel].setX(int);");
		System.out.println("\t\t<- [:Colonel].setX():void;");
		this.x = x2;
	}
	public void setY(int y2){
		System.out.println("\t\t-> [:Colonel].setY(int);");
		System.out.println("\t\t<- [:Colonel].setY():void;");
		this.y = y2;
	}
	...
	public void move(int dx,int dy){
		...
		LinkedList<Entity> temp2 = sgg.getList();
		int temp = C_Col(this, temp2); 
		
		if(temp!=0){
			temp2.get(temp).onCollision(dx, dy, temp);
		}
